...
>
> ✅ **2026-07-26 已实施完毕，本文自此只作背景材料，实施结果与最终定案见 `HANDOFF.md` §38。**
> 与本文有出入的三处，以 §38 为准：①个股定为 `/tech?s=nvda` 而不是 `/tech/nvda`（相对路径 + Pages
> 的 200 兜底会让二级路径静默裸奔）；②`_redirects` **故意不写** `/* /index.html 200`（生产实测
> Pages 默认就这么兜，加一条无法本地验证的 splat 反而可能盖住真实静态资源）；③本文 §三.3 说
> 路由在 `js/app.js` 700–720 行，实际当时已在 745 行起。
