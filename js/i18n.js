...
(function () {
  ...
  const D = {
    ...
    "读数、方法论与完整信号台账永久免费公开：上线（2026-07）起为逐日事前记录，2011 至上线前为公开数据回填、可独立复现；见证链公开可查：哈希链台账、GitHub 公开发布、第三方网页存档、Zenodo DOI 检查点。": ["Readings, methodology, and the complete signal ledger are permanently free: an ex-ante daily record since launch (July 2026), reproducible public-data backfill before that; the witness chain is public: a hash-chained ledger, public GitHub releases, third-party web archives, and Zenodo DOI checkpoints.", "", "", ""],
    "官方定义：恐惧的标价指数（英文名 Fear-Price Index）是 Market Chronicle 每个交易日发布的长天期期权成本刻度：取 1 年期市场波动率（VIX1Y）在过去 3 年的百分位，读数 0 到 100，高即贵。它只描述成本，不测收益、不预测方向、不构成买卖建议。序列标识 mc.fear_price.vix1y_pct3y.v1，公式与序列身份一经公开即不静默更改，任何口径迭代都会另立新版本。它与 KAPX 指数是两条独立序列，公式不同、互不派生。曾用英文名 Fear's Price Tag、LEAPS Cost Gauge、The Price of Fear 均指同一序列。": ["Canonical definition: The Fear-Price Index (恐惧的标价指数) is a descriptive gauge of long-dated option cost published by Market Chronicle each trading day: the percentile of 1-year implied volatility (VIX1Y) over the trailing 3 years, read 0 to 100, higher meaning historically more expensive. Descriptive only: never return-tested, not a trading signal, not investment advice. Series identifier: mc.fear_price.vix1y_pct3y.v1; the formula and series identity are never changed silently, and any revision would launch a new version. It is a separate series from the KAPX Index: the two use different formulas and neither is derived from the other. Formerly published in English as Fear's Price Tag, the LEAPS Cost Gauge and The Price of Fear, all legacy aliases of this same series.", "", "", ""],
    ...
    // 金融 键在下方行业暴露区（GICS 板块名一组，与顶栏 tab 共用同一译文 Financials）
    ...
    "恐惧的标价 · 台账": ["Fear-Price Index · Ledger", "Fear-Price Index · Registre", "Fear-Price Index · Journal", "Fear-Price Index · Registro"],
    ...
    "恐惧的标价指数 · VIX1Y 三年滚动百分位（2007 年至今）": ["Fear-Price Index · VIX1Y 3-year rolling percentile (2007–present)", "", "", ""],
    ...
    "这一段量的是 30 天，上面那些量的是 1 年。同一个市场的两截时间，不能相减，也不能混着说。保费、成交结构、持仓，是三件事：一件说价格，一件说今天的成交，一件说此刻还留着的仓位：而最后那件，还是两周前的。": ["This section measures 30 days; everything above measures one year. Two different stretches of time in the same market — they cannot be subtracted from one another, nor spoken of interchangeably. And the price of protection, the structure of trading, and the positions still outstanding are three separate things: one speaks of price, one of today's flow, one of what is still held — and that last one is already two weeks old.", "", "", ""],
    ...
    // ---- 做空持仓（2026-07-18 夜）----
    "做空持仓 · 补仓天数在自身近两年中的位置": ["Short interest · where days-to-cover sits in this name's own past two years", "", "", ""],
    "这张和上一张不是一回事。": ["This card and the one above are not the same thing.", "", "", ""],
    "上面量的是当日": ["The one above measures today's ", "", "", ""],
    "流量": ["flow", "", "", ""],
    "（今天的卖单里有多少是做空），这里量的是": ["  (how much of today's selling was short); this one measures ", "", "", ""],
    "存量": ["stock", "", "", ""],
    "（此刻还有多少股在被做空）。": ["  (how many shares are still sold short right now). ", "", "", ""],
    "本表滞后约两周": ["This table lags by about two weeks", "", "", ""],
    "：每月两次结算，结算日之后约 8 个交易日才发布，所以它": [" — settled twice a month and published roughly 8 trading days after the settlement date, so it is ", "", "", ""],
    "不是今天的读数": ["not a reading of today", "", "", ""],
    "，只能当缓慢变动的背景。用的是补仓天数（持仓 ÷ 日均成交量）而非持仓股数：股数会随股本与成交量长期漂移，除以成交量之后才是「相对于这只票的流动性，空头有多拥挤」。同样地，": [" and can only serve as slow-moving background. It uses days-to-cover (short interest ÷ average daily volume) rather than raw shares short: raw share counts drift over time with share issuance and volume, and only after dividing by volume does it answer \"how crowded is the short side relative to this name's own liquidity\". Likewise, ", "", "", ""],
    "持仓高 ≠ 有人看空": ["high short interest does NOT mean someone is bearish", "", "", ""],
    ...
    "用补仓天数（持仓 ÷ 日均成交量）在自身近两年中的位置。它回答的是「相对于这只票自己的流动性，空头有多挤」：而不是「谁被做空的股数多」（大票天然多）。持仓高 ≠ 有人看空：做市商对冲、ETF 申赎套利、可转债与并购套利都会留下空头持仓。": ["Days-to-cover (short interest ÷ average daily volume), ranked within each name's own past two years. It answers “how crowded is the short side relative to this name's own liquidity” — not “who has the most shares sold short” (large caps always do). High short interest does NOT mean someone is bearish: market-maker hedging, ETF create/redeem arbitrage, convertible and merger arbitrage all leave short positions behind.", "", "", ""],
    "补仓天数分位": ["Days-to-cover percentile", "", "", ""],
    "持仓股数分位": ["Shares-short percentile", "", "", ""],
    "当日做空占比分位（流量）": ["Daily short-volume percentile (flow)", "", "", ""],
    "各标的的拥挤度百分位": ["Crowding percentile by name", "", "", ""],
    "本表滞后约两周：每月两次结算，结算日之后约 8 个交易日才发布，所以它不是今天的读数，只能当缓慢变动的背景。历史不足 24 期的不给百分位，留空。": ["This table lags by about two weeks — settled twice a month and published roughly 8 trading days after the settlement date, so it is not a reading of today and can only serve as slow-moving background. Names with fewer than 24 periods of history are left blank.", "", "", ""],
    "单票下钻": ["One Name, Up Close", "", "", ""],
    "一只票的两条命": ["Two lives of a single name", "", "", ""],
    "上面是横截面，这里是纵切面。选一只票，看它的持仓（柱）与补仓天数（线）在最近 48 个结算期里怎么走。两者常常不同向：这正是「股数」与「拥挤度」不是一回事的直接证据。": ["The section above is a cross-section; this one is a slice through time. Pick a name and watch its short interest (bars) and days-to-cover (line) across the last 48 settlement periods. The two often move in opposite directions — direct evidence that “shares short” and “crowding” are not the same thing.", "", "", ""],
    "持仓与拥挤度 · 最近 48 个结算期": ["Short interest and crowding · last 48 settlement periods", "", "", ""],
    "柱为做空持仓（股数，左轴），线为补仓天数（右轴）。两轴各自缩放，只看形状不看高低对比。": ["Bars are short interest in shares (left axis); the line is days-to-cover (right axis). The two axes scale independently — read the shapes, not the relative heights.", "", "", ""],
    "一个被数据否掉的提案": ["A Proposal the Data Killed", "", "", ""],
    "「流量与存量背离」，是零": ["“Flow versus stock divergence” is zero", "", "", ""],
    "原本的设想是：某天做空成交占比冲高、但持仓没增，就说明那是对冲或套利而非建仓。听起来很顺：实测不成立。537 个结算窗口，横轴是流量，纵轴是持仓变化。按开关换配对方式。": ["The original idea: when the daily short-volume share spikes but short interest does not rise, that must be hedging or arbitrage rather than position-building. It sounds tidy — and it does not hold up. 537 settlement windows; flow on the horizontal axis, change in short interest on the vertical. Use the switch to change how the two are paired.", "", "", ""],
    "流量水平 vs 持仓变化（原提案）": ["Flow level vs change in short interest (original proposal)", "", "", ""],
    "流量变化 vs 持仓变化（同类对同类）": ["Change in flow vs change in short interest (like for like)", "", "", ""],
    "每个结算窗口一个点": ["One dot per settlement window", "", "", ""],
    "红点为个股，蓝点为 ETF（SPY / QQQ）。": ["Red dots are single stocks; blue dots are the ETFs (SPY / QQQ).", "", "", ""],
    "SPEARMAN 相关系数 · 全样本合并": ["SPEARMAN CORRELATION · POOLED SAMPLE", "", "", ""],
    "显著性": ["Significance", "", "", ""],
    "四只显著，但校正后一只不剩": ["Four are significant; after correction, none survive", "", "", ""],
    ...
    "当日做空成交占比在自身三年历史中的位置。这是流量，与另外两档的存量口径不同。": ["Where the day's short-volume share sits within its own three-year history. This is flow — a different measure from the two stock-based views.", "", "", ""],
    "补仓天数 = 做空持仓 ÷ 日均成交量，除掉了规模，读的是相对拥挤度。": ["Days-to-cover = short interest ÷ average daily volume. Dividing out size leaves relative crowding.", "", "", ""],
    "持仓股数的分位。注意它有非平稳问题：多数票同时逼近高位，多半是尺子的问题不是市场的问题。": ["Percentile of raw shares short. Note this measure is non-stationary — when most names approach a high at once, that usually says more about the yardstick than about the market.", "", "", ""],
    "未显示：": ["Not shown: ", "", "", ""],
    "（历史不足，不给百分位：宁可不出数，也不出假数）": [" (insufficient history — no percentile is given; better no figure than a false one)", "", "", ""],
    "持仓": ["short interest", "", "", ""],
    "补仓天数": ["days-to-cover", "", "", ""],
    "股数分位": ["shares percentile", "", "", ""],
    "拥挤度分位": ["crowding percentile", "", "", ""],
    "流量变化与持仓变化的相关": ["Correlation of flow change with short-interest change", "", "", ""],
    "置换检验": ["permutation test", "", "", ""],
    "（p<0.05，但 Bonferroni 校正后不存活）": [" (p<0.05, but does not survive Bonferroni correction)", "", "", ""],
    "（不显著）": [" (not significant)", "", "", ""],
    "该标的在本窗口内无数据（代码复用切断后历史不足）": ["No data for this name in this window (insufficient history after the ticker-reuse cutoff)", "", "", ""],
    "该标的在本窗口内无数据": ["No data for this name in this window", "", "", ""],
    ...
    "纵轴越高越贵（0–100），50 为中性；绿色为可复现回测段，猩红为发布后逐日记录的前向台账（自 2026-07-13）；底部滑块可拖拽缩放": ["Higher on the y-axis = more expensive (0–100), 50 is neutral; green is the reproducible backtest segment, scarlet is the forward ledger recorded daily since launch (2026-07-13); drag the slider below to zoom.", "", "", ""],
    ...
    "波动率点": ["Vol points", "", "", ""],
    // ---- 期货双源（VX 曲线 + COT，2026-07-17）----
    "恐惧的远期价目表 · VIX 期货逐月结算价": ["Fear's forward price list · VIX futures settlements by month", "", "", ""],
    "真金白银交易出来的恐惧曲线，与上方指数轨互证：实线 = 最新结算，向右上倾斜 = 越远越贵；虚线 = 一周前对照；首点 = VIX 现值。期货接近 24 小时交易，官方账本每天只记这一笔结算价。": ["The fear curve as traded with real money, cross-checking the index track above: solid = latest settlement, sloping up = the further out, the dearer; dashed = one week earlier; first point = spot VIX. Futures trade nearly 24 hours; the official ledger records one settlement per day.", "", "", ""],
    "谁在押注恐惧 · 杠杆基金在 VIX 期货的净头寸": ["Who is betting on fear · Leveraged funds' net position in VIX futures", "", "", ""],
    "CFTC 官方持仓（2006 年至今）：每周二数据、周五发布，滞后三天；零线以下 = 净做空恐惧（历史常态），以上 = 净做多；虚线 = 资管机构对照 · 底部滑块可拖拽缩放": ["Official CFTC positioning (since 2006): as of each Tuesday, published Friday, three days behind; below zero = net short fear (the historical norm), above = net long; dashed = asset managers for contrast · drag the bottom slider to zoom", "", "", ""],
    "VIX 现值": ["Spot VIX", "", "", ""],
    "最新结算": ["Latest settlement", "", "", ""],
    "一周前": ["One week earlier", "", "", ""],
    "净头寸（手）": ["Net contracts", "", "", ""],
    ...
    "上图：CNN（黄，左轴 0–100）· VIX（紫，左轴）· 纳指 100（蓝，右轴，对数）｜下图：K 指数（红，左轴 0–12），K < 1 区间高亮": [
      "Top: CNN (gold, left 0–100), VIX (purple, left), NDX (blue, right, log). Bottom: KAPX Index (red, 0–12), K < 1 zones highlighted.",
  ...
  const P = [
    ...
    [/^数据截至 (.+)（结算日） · (.+) · 每月两次结算，结算日后约 8 个交易日发布$/,
      ["Data as of $1 (settlement date) · $2 · settled twice monthly, published ~8 trading days after settlement",
       "", "", ""]],
    [/^数据截至 (.+) · (.+) · 每周六自动更新$/,
