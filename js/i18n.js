...
(function () {
  ...
  const D = {
    ...
    "净值 = 1 元照规则操作后变成几元，尾标 ×N 即 1 元变 N 元。信号首日买入纳指 100、持有 60 个交易日后离场，持有期内新信号跳过；两条虚线为同期一直持有纳指与标普。空仓期记零、不计成本与滑点，完整口径见方法论": ["The curve is simply what $1 becomes under the rules — an end label of ×N means $1 grew to $N. Buy the Nasdaq-100 at each signal's first close, exit after 60 trading days, skip signals inside a holding period; the two dashed lines are buy-and-hold Nasdaq and S&P. Cash earns zero, no costs or slippage — full rules in the Methodology", "", "", ""],
    ...
    "入场与离场": ["Entries & Exits", "", "", ""],
    "信号之后，拿多久最合适？": ["After a signal, how long should you hold?", "", "", ""],
    "持有期矩阵：五个视界的胜率与中位收益": ["The holding-period matrix: win rates and median returns across five horizons", "", "", ""],
    "每次信号首日入场，之后 20 / 40 / 60 / 120 / 250 个交易日的胜率与中位涨跌幅（纳指 / 标普双锚）；较新的信号尚未走完长视界，不计入该列": ["Enter at each signal's first close; win rate and median change 20 / 40 / 60 / 120 / 250 trading days later (Nasdaq / S&P dual anchors); recent signals that haven't completed a horizon are excluded from that column", "", "", ""],
    ...
    "上图：CNN（黄，左轴 0–100）· VIX（红，左轴）· 纳指 100（蓝，右轴，对数）｜下图：K 指数（红，左轴 1–10），K < 1 区间高亮": [
      "Top: CNN (gold, left 0–100), VIX (red, left), NDX (blue, right, log). Bottom: KAPX Index (red, 1–10), K < 1 zones highlighted.",
      "Haut : CNN (or), VIX (rouge), NDX (bleu, log). Bas : indice KAPX (rouge, 1–10), zones K < 1 en surbrillance.",
      "Oben: CNN (gold), VIX (rot), NDX (blau, log). Unten: KAPX Index (rot, 1–10), K < 1 hervorgehoben.",
      "Arriba: CNN (oro), VIX (rojo), NDX (azul, log). Abajo: índice KAPX (rojo, 1–10), zonas K < 1 resaltadas."],
